...
{
    public partial class UC_ТоварА : UserControl
    {
        int selectedRow;
        private DB db;
        ...
        private void UC_ТоварА_Load(object sender, EventArgs e)
        {
            this.товарTableAdapter.Fill(this.нестеровКурсоваяDataSet.Товар);
        }
        private void guna2Button2_Click(object sender, EventArgs e)
        {
            Validation validator = new Validation();
            string firstName = guna2TextBox2.Text;
            string number = guna2TextBox3.Text;
            string text = guna2TextBox4.Text;
            if (!validator.ValidateLastName(firstName))
            {
                MessageBox.Show("Некорректное наименование компании", "Ошибка");
                return;
            }
            if (!validator.ValidateText(text))
            {
                MessageBox.Show("Некорректное описание", "Ошибка");
                return;
            }
            if (!validator.ValidateNumber(number))
            {
                MessageBox.Show("Некорректное цена", "Ошибка");
                return;
            }
            var queryAdd = $"insert into Товар (Наименование_товара, Цена_товара, Описание_товара)" +
        ...
        {
            var queryUp = $"update Товар set Наименование_товара = '{guna2TextBox2.Text}'," +
                $" Цена_товара = '{guna2TextBox3.Text}'," +
                $" Описание_товара = '{guna2TextBox4.Text}' where Код_товара = {guna2TextBox5.Text}";
            db.queryExecute(queryUp);
            LoadDataIntoDataGridView();
        }
        private void guna2Button1_Click(object sender, EventArgs e)
        {
            var queryDel = $"delete Товар where Код_товара = {guna2TextBox1.Text} ";
        ...
        private void guna2TextBoxSearch_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                {
                    товарBindingSource.Filter = string.Format("{0}='{1}'",
