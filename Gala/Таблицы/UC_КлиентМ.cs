...
{
    public partial class UC_КлиентМ : UserControl
    {
    ...
    " Catalog=НестеровКурсовая;Integrated Security=True");
        ...
        private void guna2TextBoxSearch_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                {
                    клиентBindingSource.Filter = string.Empty;
                }
                else
                {
                    клиентBindingSource.Filter = string.Format("{0}='{1}'",
                        guna2ComboBox1.Text, guna2TextBoxSearch.Text);
                }
            }
        }
        private void UC_КлиентМ_Load(object sender, EventArgs e)
        {
            this.клиентTableAdapter.Fill(this.нестеровКурсоваяDataSet.Клиент);
        }
